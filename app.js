...
const app = {
  ...
  async searchAssets(q){
    ...
    const uni=await state.cache.tradeUniverseP;
    let res=uni.filter(a=>a.symbol.startsWith(q)).concat(uni.filter(a=>!a.symbol.startsWith(q)&&(a.symbol.includes(q)||(a.name||"").toUpperCase().includes(q)))).slice(0,25);
    if(!res.length && /^[A-Z0-9.\-]{1,12}$/.test(q)) res=[{symbol:q,name:"Usar este símbolo tal cual",type:"stock"}];
    if(!res.length){ box.innerHTML=`<div class="sr-empty">Sin coincidencias para "${esc(q)}"</div>`; box.classList.remove("hidden"); return; }
    box.innerHTML=res.map(a=>`<div class="sr-item" onclick="app.pickSym('${esc(a.symbol)}')">
      <div><b class="mono">${esc(a.symbol)}</b> <span class="sr-name">${esc(a.name||"")}</span></div>
      <span class="sr-ex">${a.type==="crypto"?"Cripto":a.type==="etf"?"ETF":"Acción"}</span>
    </div>`).join("");
    box.classList.remove("hidden");
  },
